        ...
        <div class="space-y-4 text-lg text-gray-700">
          <div v-if="websiteSettings.hospitalName" class="flex items-start">
            ...
            <div>
              ...
              <p v-if="websiteSettings.slogan" class="text-sm text-gray-500">
                {{ websiteSettings.slogan }}
              </p>
            </div>
          </div>
          <div v-if="websiteSettings.address" class="flex items-start">
            ...
            <p>
              {{ websiteSettings.address }} {{ websiteSettings.zipCode }}
              {{ websiteSettings.province }}
            </p>
          </div>
          <div v-if="websiteSettings.phoneMain" class="flex items-center">
            ...
            <p>
              โทรศัพท์:
              <a :href="'tel:' + websiteSettings.phoneMain" class="text-blue-600 hover:underline">{{
                websiteSettings.phoneMain
              }}</a>
            </p>
          </div>
          <div v-if="websiteSettings.phoneEmergency" class="flex items-center">
            ...
            <p>
              เบอร์ฉุกเฉิน:
              <a
                :href="'tel:' + websiteSettings.phoneEmergency"
                class="text-blue-600 hover:underline"
                >{{ websiteSettings.phoneEmergency }}</a
              >
            </p>
          </div>
          <div v-if="websiteSettings.emailMain" class="flex items-center">
            ...
            <p>
              อีเมล:
              <a
                :href="'mailto:' + websiteSettings.emailMain"
                class="text-blue-600 hover:underline"
                >{{ websiteSettings.emailMain }}</a
              >
            </p>
          </div>
          <div class="pt-4 border-t border-gray-200 mt-4" v-if="hasSocialMedia">
            ...
            <div class="flex space-x-4">
              <a
                v-if="websiteSettings.facebookUrl"
                :href="websiteSettings.facebookUrl"
                target="_blank"
                class="text-blue-600 hover:text-blue-800 text-3xl transition-colors duration-300"
              >
                <i class="fab fa-facebook-square"></i>
              </a>
              <a
                v-if="websiteSettings.lineId"
                :href="getLineUrl(websiteSettings.lineId)"
                target="_blank"
                class="text-green-500 hover:text-green-700 text-3xl transition-colors duration-300"
              >
                <i class="fab fa-line"></i>
              </a>
              <a
                v-if="websiteSettings.twitterUrl"
                :href="websiteSettings.twitterUrl"
                target="_blank"
                class="text-blue-400 hover:text-blue-600 text-3xl transition-colors duration-300"
              >
                <i class="fab fa-twitter-square"></i>
              </a>
              <a
                v-if="websiteSettings.youtubeUrl"
                :href="websiteSettings.youtubeUrl"
                target="_blank"
                class="text-red-600 hover:text-red-800 text-3xl transition-colors duration-300"
              >
...
const hasSocialMedia = computed(() => {
  return (
    websiteSettings.value.facebookUrl ||
    websiteSettings.value.lineId ||
    websiteSettings.value.twitterUrl ||
    websiteSettings.value.youtubeUrl
  )
})
