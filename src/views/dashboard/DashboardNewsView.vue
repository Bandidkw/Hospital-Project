  ...
  <div class="p-6 bg-white rounded-lg shadow-md">
    ...
    <div class="card bg-gray-50 p-6 rounded-lg shadow-inner mb-8">
      ...
      <form @submit.prevent="saveNews" class="space-y-4">
        ...
        <div>
          <label for="newsDate" class="block text-sm font-medium text-gray-700">วันที่เผยแพร่:</label>
          <input type="date" id="newsDate" v-model="currentNews.date" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500" required>
        </div>
        <div>
          <label for="newsImage" class="block text-sm font-medium text-gray-700">รูปภาพประกอบ (URL):</label>
          <input type="text" id="newsImage" v-model="currentNews.imageUrl" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500">
          <p class="text-xs text-gray-500 mt-1">สามารถใส่ URL รูปภาพ หรือเว้นว่างไว้</p>
        </div>
        <div class="flex items-center">
          <input id="isPublished" type="checkbox" v-model="currentNews.isPublished" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
          ...
          <tbody class="text-gray-600 text-sm font-light">
            <tr v-for="(news, index) in newsList" :key="news.id" class="border-b border-gray-200 hover:bg-gray-50">
              <td class="py-3 px-6 text-left">{{ index + 1 }}</td>
              <td class="py-3 px-6 text-left">{{ news.title }}</td>
              <td class="py-3 px-6 text-left">{{ news.date }}</td>
              <td class="py-3 px-6 text-left">
                <span :class="{'bg-green-200 text-green-800': news.isPublished, 'bg-red-200 text-red-800': !news.isPublished}" class="py-1 px-3 rounded-full text-xs">
                  {{ news.isPublished ? 'เผยแพร่แล้ว' : 'ฉบับร่าง' }}
                </span>
              </td>
              <td class="py-3 px-6 text-center">
                <button @click="editNews(news)" class="bg-yellow-500 text-white px-3 py-1 rounded-md text-xs hover:bg-yellow-600 transition duration-300 mr-2">
                  <i class="fas fa-edit"></i> แก้ไข
                </button>
                <button @click="confirmDeleteNews(news.id)" class="bg-red-500 text-white px-3 py-1 rounded-md text-xs hover:bg-red-600 transition duration-300">
...
const newsList = ref<NewsItem[]>([
  { id: 1, title: 'ประกาศวันหยุดราชการ', content: 'เนื่องในวัน...', date: '2024-07-28', isPublished: true, imageUrl: 'https://via.placeholder.com/150' },
  { id: 2, title: 'ข่าวรับสมัครบุคลากร', content: 'โรงพยาบาล...', date: '2024-07-20', isPublished: false, imageUrl: '' },
]);
