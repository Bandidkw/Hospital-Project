<template>
  <div class="p-6 bg-white rounded-lg shadow-md">
    ...
    <p class="text-gray-700 mb-6">
      หน้านี้ใช้สำหรับกำหนดและจัดการรูปภาพเฉพาะสำหรับแต่ละหน้าของเว็บไซต์.
    </p>
    ...
    <div class="card bg-gray-50 p-6 rounded-lg shadow-inner mb-8">
      ...
      <form @submit.prevent="assignImageToPage" class="space-y-4">
        <div>
          ...
          <!-- Custom Dropdown สำหรับเลือกหน้า -->
          <div class="relative mt-1">
            <button
              type="button"
              @click="isPageDropdownOpen = !isPageDropdownOpen"
              @blur="onBlurPage"
              class="w-full bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-left hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 flex items-center justify-between"
            >
              <span v-if="assignForm.page" class="flex items-center">
                <i :class="getPageIcon(assignForm.page)" class="mr-2"></i>
                {{ getPageName(assignForm.page) }}
              </span>
              <span v-else class="text-gray-400">-- เลือกหน้า --</span>
              <i
                class="fas fa-chevron-down text-gray-400 ml-2 transition-transform duration-200"
                :class="{ 'rotate-180': isPageDropdownOpen }"
              ></i>
            </button>

            <!-- Dropdown Menu -->
            <div
              v-if="isPageDropdownOpen"
            ...
            >
              <div
                ...
                :key="page.value"
                @mousedown.prevent="selectPage(page.value)"
                class="px-4 py-2.5 hover:bg-blue-50 cursor-pointer transition-colors duration-150 flex items-center justify-between group"
              >
                <div class="flex items-center">
                  <span
                    class="w-2 h-2 rounded-full mr-3 transition-colors duration-200"
                    :class="
                      assignForm.page === page.value
                        ? 'bg-blue-500'
                        : 'bg-gray-300 group-hover:bg-blue-400'
                    "
                  ></span>
                  <i
                    :class="getPageIcon(page.value)"
                    class="mr-2 text-gray-600 group-hover:text-blue-600"
                  ></i>
                  <span class="text-gray-700 group-hover:text-blue-600">{{ page.text }}</span>
                </div>
                <i v-if="assignForm.page === page.value" class="fas fa-check text-blue-500"></i>
              </div>
            </div>
          </div>
        </div>
        <div>
          <label for="pageImageUpload" class="block text-sm font-medium text-gray-700"
            >อัปโหลดรูปภาพใหม่ (ถ้ามี)</label
          >
          <input
            type="file"
            id="pageImageUpload"
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
          ...
          <tbody class="text-gray-600 text-sm font-light">
            <tr
              v-for="pageImage in pageImagesList"
              :key="pageImage.page"
            ...
            <tr v-if="pageImagesList.length === 0">
              <td colspan="3" class="py-8 text-center text-gray-500">
                ยังไม่มีรูปภาพที่กำหนดสำหรับหน้าใดๆ.
              </td>
            </tr>
          </tbody>
        </table>
...
// ฟังก์ชันสำหรับเลือกหน้าจาก dropdown
const selectPage = (pageValue: string) => {
  assignForm.value.page = pageValue
...
// ฟังก์ชันสำหรับแสดงไอคอนตามประเภทหน้า
const getPageIcon = (pageValue: string) => {
  const icons: Record<string, string> = {
    home: 'fas fa-home text-blue-500',
    about: 'fas fa-info-circle text-green-500',
    'services/outpatient': 'fas fa-hospital-user text-purple-500',
    news: 'fas fa-newspaper text-orange-500',
  }
  return icons[pageValue] || 'fas fa-file text-gray-500'
}
...
const assignImageToPage = () => {
  ...
  console.log(
    'Assigning image to page:',
    assignForm.value.page,
    assignForm.value.file?.name || 'existing image',
  )
