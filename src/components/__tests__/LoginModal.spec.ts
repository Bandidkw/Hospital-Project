...
describe('LoginModal', () => {
  ...
  it('clears form when modal is closed', async () => {
    ...
    // Re-open modal to check if form is cleared
    await wrapper.setProps({ isOpen: true })
    await wrapper.vm.$nextTick()

    const usernameInputAfter = wrapper.find('input[type="text"]')
    const passwordInputAfter = wrapper.find('input[type="password"]')

    expect((usernameInputAfter.element as HTMLInputElement).value).toBe('')
    expect((passwordInputAfter.element as HTMLInputElement).value).toBe('')
  })

  it('resets form when modal is opened', async () => {
    const wrapper = createWrapper({ isOpen: false })

    // Open modal first to get inputs
    await wrapper.setProps({ isOpen: true })
    await wrapper.vm.$nextTick()
    ...
    await usernameInput.setValue('olduser')
    await passwordInput.setValue('oldpass')
