    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm relative">
        <button
          type="button"
          @click="closeModal"
          class="absolute top-3 right-3 text-gray-500 hover:text-gray-800 transition p-1 rounded-full hover:bg-gray-100"
...
<style scoped>
/*
  กำหนด Transition Classes สำหรับชื่อ "fade"
  (ใช้สำหรับการ Fade-in/Fade-out ของพื้นหลังและตัว Modal พร้อมกัน)
*/

/* สถานะเริ่มต้นเมื่อ Modal กำลังจะเข้า (Entering) และสถานะสุดท้ายเมื่อกำลังจะออก (Leaving) */
.fade-enter-from,
.fade-leave-to {
  opacity: 0;
}

/* กำหนดระยะเวลาและความเร็วของ Transition */
.fade-enter-active,
.fade-leave-active {
  transition: opacity 0.3s ease; /* กำหนดให้ใช้เวลา 0.3 วินาที */
}
</style>
